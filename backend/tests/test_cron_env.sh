...
ALL_VARS=(OLLAMA_BASE_URL OLLAMA_MODEL DGX_HOST LLM_CONNECT_TIMEOUT LLM_RESPONSE_TIMEOUT \
...
check "all vars written when set" \
  "grep -q '^OLLAMA_BASE_URL=http://192.168.1.102:11434' \"\$OUTPUT_FILE\" && \
   ...
   grep -q '^LLM_CONNECT_TIMEOUT=2' \"\$OUTPUT_FILE\" && \
   grep -q '^LLM_RESPONSE_TIMEOUT=5' \"\$OUTPUT_FILE\" && \
