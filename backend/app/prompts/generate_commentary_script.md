...
与えられた1記事の入力本文から、解説音声の台本を日本語で作ってください。
入力本文は取得できた範囲のみであり、記事全体とは限りません。長文記事では先頭から最大1,500文字に切り詰められている場合があります。
解説は入力本文に含まれる情報だけを根拠にし、未取得の後半部分や欠落情報の内容・結論・反論・注意点を推測して補完しないでください。
...
- 一人語りの文体を厳守すること：
  ...
  - 対話相手を前提とする語尾（「〜ですか」「〜ですね」「〜ことなのですね」「〜なのでしょうか」など、相手の発言を受ける相槌・確認表現）を一切使わない
  - 良い例：「このニュースの背景には…」「注目すべき点は…」「具体的には…」
  - 悪い例：「〜ですね」「〜ですか」「〜ことなのですね」「〜なのでしょうか」
...
- news の各 line は60〜120文字程度とする。一文で言い切って終わらせず、事実とその意味づけをセットで述べる
- 行数を満たすために内容の薄い行で水増しせず、記事の別の論点を取り上げて行数を確保する
...
入力記事は本文全体が与えられます。冒頭だけを読んで要約するのではなく、記事全体を通読してから構成してください。

- 記事の主要な論点は、冒頭の数段落に限らず**全体から**拾い上げる
- 記事に書かれている数値・固有名詞・日付・引用・具体例は、可能な限り台本に反映する
- 記事にある反論・注意点・限界（「ただし〜」「一方で〜」に続く内容）を省略しない
- 記事を一段抽象化した「まとめ」で済ませず、記事を読んでいないリスナーが元記事とほぼ同じ情報を得られる密度を目指す
- 記事に書かれていない事実を補ってはならない
