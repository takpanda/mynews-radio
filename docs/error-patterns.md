...
現状の `replacement_table.py` は **完全一致（正規表現 OR による最長一致）** を使用している。

**判断**: 辞書DB連携後も、生成時のマッチング方式は **完全一致（最長一致）** を維持する。前方一致や部分一致を許容すると、不自然な読み替えが発生するリスクがある。大文字小文字の区別も現状維持。
...
| E-A-04 | 部分一致による不自然な置換 | 完全一致（正規表現ORの最長一致） | 現状維持。完全一致（最長一致）を継続。 | 前方一致や部分一致は不自然な読み替えの原因となるため許容しない。 |
...
| E-A-12 | 原稿テキストが `None` または空文字 | `original_text = line.get("text", "")` で空文字保証。`apply_replacements("")` は `""` を返す。 | 同上。 | 正常動作。エラーではない。 |
...
**判断**: 完全一致（最長一致）を維持。部分一致や前方一致は、意図しない置換の原因となるため許容しない。
