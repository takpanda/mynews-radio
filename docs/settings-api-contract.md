...
カテゴリ別設定で選択できる女性MCは `female_mc_candidates` 候補マスタで管理する。初期データとして既存の `female`, `morigawa` を投入し、Fish S2 Pro の `/health` 応答には依存しない。設定は `user_settings.fishs2pro_category_female_voices` に JSON として保存し、入力に含まれないカテゴリは未設定として扱う。候補名はFish S2 Proへ送信するボイス名として登録するため、候補追加時に音声サーバーの一覧取得は行わない。
